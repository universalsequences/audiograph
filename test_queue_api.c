...
int main() {
  ...
  osc_state[OSC_INC] = 440.0f / 48000.0f; // frequency to phase increment
  osc_state[OSC_PHASE] = 0.0f;
  
  float *gain_state = malloc(sizeof(float) * GAIN_MEMORY_SIZE);
  gain_state[GAIN_VALUE] = 0.5f;

  int osc_id = add_node(lg, OSC_VTABLE, osc_state, "test_osc", 0, 1);    // Osc: 0 inputs, 1 output
  int gain_id = add_node(lg, GAIN_VTABLE, gain_state, "test_gain", 1, 1); // Gain: 1 input, 1 output
  ...
  bool connect_result = connect(lg, osc_id, 0, gain_id, 0);
  assert(connect_result == true);
  printf("✓ Connected osc->gain using pre-allocated IDs\n");
  
  bool connect_to_dac = connect(lg, gain_id, 0, lg->dac_node_id, 0);
