...
static void dual_output_process(float *const *inputs, float *const *outputs, int block_size, void *state) {
...
void test_complex_topology() {
    ...
    int node1 = add_node(lg, DUAL_OUTPUT_VTABLE, sizeof(DualOutputState), "dual_node1", 0, 2);
    assert(node1 >= 0);
    ...
    printf("Node 3 - Inputs: %d, Outputs: %d\n", lg->nodes[node3].nInputs, lg->nodes[node3].nOutputs);
    ...
    printf("  Node 3 after second connection - inEdgeId[0]: %d, fanin_sum_node_id[0]: %d\n", 
    ...
    // With auto-summing, destination nodes have indegree=1 (connected to SUM node)
    // The SUM nodes themselves have the multi-input indegrees
    assert(lg->indegree[node1] == 0);
    assert(lg->indegree[node2] == 1);  
    assert(lg->indegree[node3] == 1);  // Connected to auto-generated SUM node
    assert(lg->indegree[lg->dac_node_id] == 1);  // Connected to auto-generated SUM node
    ...
    // Node 3 receives 15.0 (from Node 1 port 1) + 20.0 (from Node 2), sums to 35.0, multiplies by 3.0 = 105.0
    // DAC receives 20.0 (from Node 2) + 105.0 (from Node 3) = 125.0
