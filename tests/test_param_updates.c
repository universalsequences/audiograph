...
void state_output_process(float* const* in, float* const* out, int n, void* memory) {
...
const NodeVTable STATE_OUTPUT_VTABLE = {
    .process = state_output_process,
    .init = NULL,
    .migrate = NULL
};

// Helper function to add state output node to live graph
int live_add_state_output(LiveGraph* lg, float initial_value, const char* name) {
...
void test_param_updates() {
    ...
    ParamMsg invalid_msg = {
        .logical_id = 999, // Invalid node ID
        .idx = 0,
        .fvalue = 5.0f
    };

    push_result = params_push(lg->params, invalid_msg);
    assert(push_result); // Should still push successfully (handled during processing)
    printf("✓ Invalid node ID parameter push accepted (will be ignored during processing)\n");
