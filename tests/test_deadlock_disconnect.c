...
static void src2(float *const *in, float *const *out, int n, void *m) {
  (void)in; (void)m; for (int i=0;i<n;i++){ out[0][i]=A; out[1][i]=B; }
}
...
static void dup22(float *const *in, float *const *out, int n, void *m){
  (void)m; if (in[0]) memcpy(out[0], in[0], (size_t)n*sizeof(float)); else memset(out[0],0,(size_t)n*sizeof(float));
...
int main(){
  ...
  // Disconnect pass -> DAC[0]; previously this caused deadlock when indegree drifted
