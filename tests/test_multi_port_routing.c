...
// Dual output generator: ignores inputs, outputs 0.25 on port 0 and 0.5 on port 1
#define DUAL_OUTPUT_MEMORY_SIZE 0 // No state needed

void dual_output_process(float *const *in, float *const *out, int n, void *memory) {
...
void dual_input_sum_process(float *const *in, float *const *out, int n, void *memory) {
    (void)memory; // No state
    ...
    if (n > 0) {
        printf("DUAL_SUM_DEBUG: in[0][0]=%.6f, in[1][0]=%.6f\n", 
               input_0 ? input_0[0] : -999.0f, 
               input_1 ? input_1[0] : -999.0f);
    }
...
const NodeVTable DUAL_OUTPUT_VTABLE = {
...
const NodeVTable DUAL_INPUT_SUM_VTABLE = {
    .process = dual_input_sum_process,
    .init = NULL,
    .reset = NULL,
    .migrate = NULL
};
...
void test_multi_port_routing() {
    ...
    printf("DEBUG: DAC structure - inputs:%d outputs:%d\n", 
           lg->nodes[lg->dac_node_id].nInputs, lg->nodes[lg->dac_node_id].nOutputs);
    ...
    if (connect1 && connect2 && connect_dac) {
        ...
        for (int i = 0; i < 5; i++) {
            if (fabsf(output_buffer[i] - expected) >= 0.001f) {
                printf("ERROR: Sample %d: got %.6f, expected %.6f (diff=%.6f)\n", 
                       i, output_buffer[i], expected, fabsf(output_buffer[i] - expected));
                all_correct = false;
            }
        }
        ...
        printf("  Connection results: node1->node2[0]=%s, node1->node2[1]=%s, node2->DAC=%s\n",
               connect1 ? "OK" : "FAIL", connect2 ? "OK" : "FAIL", connect_dac ? "OK" : "FAIL");
    }
    ...
    printf("DEBUG: Node1 orphan status: %s, indegree: %d\n", 
           lg->is_orphaned[node1] ? "ORPHANED" : "CONNECTED", lg->indegree[node1]);
    printf("DEBUG: Node2 orphan status: %s, indegree: %d\n", 
           lg->is_orphaned[node2] ? "ORPHANED" : "CONNECTED", lg->indegree[node2]);
    printf("DEBUG: DAC orphan status: %s, indegree: %d\n", 
           lg->is_orphaned[lg->dac_node_id] ? "ORPHANED" : "CONNECTED", lg->indegree[lg->dac_node_id]);
    
    // Verify edge connections
    printf("DEBUG: Node1 output edges: [0]=%d, [1]=%d\n", 
           node1_rt->outEdgeId[0], node1_rt->outEdgeId[1]);
    printf("DEBUG: Node2 input edges: [0]=%d, [1]=%d\n", 
           node2_rt->inEdgeId[0], node2_rt->inEdgeId[1]);
    ...
    if (node1_rt->outEdgeId[0] >= 0 && node1_rt->outEdgeId[1] >= 0 &&
        ...
        printf("  Edge %d (0.25 path): %.6f\n", edge_0_id, lg->edges[edge_0_id].buf[0]);
        printf("  Edge %d (0.5 path): %.6f\n", edge_1_id, lg->edges[edge_1_id].buf[0]);
        ...
        if (edge0_ok && edge1_ok) {
            ...
            printf("✗ Edge buffer verification failed: edge0=%.6f (exp 0.25), edge1=%.6f (exp 0.5)\n",
...
void test_workaround_with_intermediate_nodes() {
    ...
    bool apply_result = apply_graph_edits(lg->graphEditQueue, lg);
    assert(apply_result);
    printf("✓ Created workaround chain: dual_output → gain1,gain2 → sum_node → DAC\n");
    ...
    bool connect5 = apply_connect(lg, sum_node, 0, lg->dac_node_id, 0); // Sum:0 → DAC
    ...
    if (works) {
        printf("✓ WORKAROUND WORKS! Output = %.3f (intermediate nodes fix the issue)\n", output_buffer[0]);
    } else {
