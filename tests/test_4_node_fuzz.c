...
static bool validate_graph_state(Edge *edges, int edge_count, const char *test_name) {
    ...
    int active_count = 0;
    for (int i = 0; i < edge_count; i++) {
        if (edges[i].is_active) active_count++;
    }
    ...
    if (!output_correct || !indegrees_valid) {
        ...
        if (lg->indegree[lg->dac_node_id] == 0 && fabs(actual - expected) > 0.001f) {
            ...
            if (!found_connections) {
                printf("     ❌ NO actual DAC input connections found in graph data!\n");
            }
...
static void test_disconnection_permutation(Edge *edges, int edge_count, int depth, 
                                          char *permutation_name) {
    total_permutations++;
    
    // Validate current state
    char test_name[256];
    snprintf(test_name, sizeof(test_name), "Perm_%d_%s", total_permutations, permutation_name);
    ...
    for (int i = 0; i < edge_count; i++) {
        if (!edges[i].is_active) continue; // Skip already disconnected edges
        
        // Disconnect this edge
        printf("🔌 Disconnecting %s (permutation %d, depth %d)\n", 
               edges[i].name, total_permutations, depth + 1);
        
        bool disconnect_success = graph_disconnect(lg, edges[i].src_node, edges[i].src_port,
        ...
        if (disconnect_success) {
            ...
            char new_name[512];
            snprintf(new_name, sizeof(new_name), "%s-%s", permutation_name, edges[i].name);
            ...
            bool reconnect_success = graph_connect(lg, edges[i].src_node, edges[i].src_port,
                                                  edges[i].dst_node, edges[i].dst_port);
...
int main() {
    ...
    printf("=============================\n\n");
    printf("This test will disconnect edges in EVERY possible order and validate\n");
    printf("the graph state at each step. With 6 edges, there are 6! = 720 possible\n");
    printf("disconnection sequences, plus intermediate states = 1956 total tests.\n\n");
