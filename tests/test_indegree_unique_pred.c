...
static void src2(float *const *in, float *const *out, int n, void *m) {
  (void)in; (void)m;
  for (int i = 0; i < n; i++) { out[0][i] = A; out[1][i] = B; }
}
static const NodeVTable SRC2 = {.process=src2};

static void passt(float *const *in, float *const *out, int n, void *m) {
  (void)m; if (in[0]) memcpy(out[0], in[0], (size_t)n*sizeof(float));
  else memset(out[0], 0, (size_t)n*sizeof(float));
}
static const NodeVTable PASS = {.process=passt};

static void dup22(float *const *in, float *const *out, int n, void *m) {
  (void)m;
  if (in[0]) memcpy(out[0], in[0], (size_t)n*sizeof(float)); else memset(out[0],0,(size_t)n*sizeof(float));
  if (in[1]) memcpy(out[1], in[1], (size_t)n*sizeof(float)); else memset(out[1],0,(size_t)n*sizeof(float));
}
static const NodeVTable DUP22 = {.process=dup22};

static int count_unique_preds(LiveGraph *lg, int dst) {
  if (dst < 0 || dst >= lg->node_count) return -1;
  ...
  for (int s = 0; s < lg->node_count; s++) {
    ...
    for (int di = 0; di < D->nInputs; di++) {
      ...
      if (eid >= 0 && eid < lg->edge_capacity && lg->edges[eid].in_use && lg->edges[eid].src_node == s) { uses = 1; break; }
    }
    uniq += uses;
  }
  return uniq;
...
int main() {
  ...
  // Wiring: src:0 -> pass -> DAC[0]; src:0 -> dup:0; src:1 -> dup:1; dup:0->DAC[0], dup:1->DAC[1]
