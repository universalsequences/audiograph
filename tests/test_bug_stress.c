...
static void initialize_reusable_graph() {
    ...
    printf("✅ Initialized reusable graph with nodes: N1=%d, N2=%d, N3=%d, N4=%d, DAC=%d\n",
           g_node1_id, g_node2_id, g_node3_id, g_node4_id, g_reused_graph->dac_node_id);
}
...
static void disconnect_all_edges() {
    ...
    // Disconnect all edges in reverse order of creation to avoid dependency issues
...
static bool run_single_iteration(int iteration) {
    ...
    // Step 1: Disconnect all edges (simulates graph reset without destroying state)
    ...
    bool step2_ok = apply_disconnect(lg, g_node4_id, 0, lg->dac_node_id, 0); // N4→DAC  
    ...
    bool step4_ok = apply_disconnect(lg, g_node3_id, 0, lg->dac_node_id, 0); // N3→DAC
...
static bool run_different_sequences(int iteration) {
    ...
    has_bug = (fabs(actual_output) < 0.001f) && (lg->indegree[lg->dac_node_id] > 0);
    
    if (has_bug) {
        printf("🐛 BUG FOUND in sequence '%s' (iteration %d)!\n", sequences[seq_index], iteration);
        printf("   Output: %.6f, DAC indegree: %d\n", actual_output, lg->indegree[lg->dac_node_id]);
    }
...
int main() {
    ...
    printf("====================================================\n\n");
    printf("Running iterations with REUSED graph instance to trigger cumulative corruption...\n");
    printf("This simulates the exact conditions that cause the 163 fuzz test failures.\n\n");
    ...
    // Fine-tune to start 3 iterations before first observed failure (iteration 124)
    int start_iteration = 121;
    int focused_iterations = 10;  // Run just 10 iterations around the corruption point
    
    printf("🔬 FOCUSED TEST: Running iterations %d-%d (around corruption point)...\n", start_iteration, start_iteration + focused_iterations - 1);
    ...
    printf("\n🔬 FOCUSED TEST: Skipping second phase to focus on critical corruption window.\n");
    ...
    if (bugs_found > 0) {
        ...
        printf("   This confirms the exact same bug from the 163 fuzz test failures.\n");
        printf("   The bug is caused by reusing the same graph across many operations.\n");
        printf("   Ready for detailed lldb analysis on the corrupted graph instance.\n");
    } else {
