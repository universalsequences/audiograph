...
int main() {
    ...
    printf("Node 1 - Inputs: %d, Outputs: %d\n", lg->nodes[node1_id].nInputs, lg->nodes[node1_id].nOutputs);
    printf("Node 2 - Inputs: %d, Outputs: %d\n", lg->nodes[node2_id].nInputs, lg->nodes[node2_id].nOutputs);
    printf("Node 3 - Inputs: %d, Outputs: %d\n", lg->nodes[node3_id].nInputs, lg->nodes[node3_id].nOutputs);
    printf("Node 4 - Inputs: %d, Outputs: %d\n", lg->nodes[node4_id].nInputs, lg->nodes[node4_id].nOutputs);
    ...
    // DAC receives: Node 2 output 1 (3) + Node 3 output 0 (2) + Node 4 output 0 (0.5) = 5.5
    
    float expected_dac = 3.0f + 2.0f + 0.5f; // 5.5
    printf("Initial DAC output: %.2f (expected: %.2f)\n", output_buffer[0], expected_dac);
    ...
    // DAC receives: Node 2 output 1 (3) + Node 3 output 0 (2) + Node 4 output 0 (0) = 5.0
    ...
    if (fabs(output_buffer[0] - expected_dac_after) < 0.001f) {
        ...
        printf("✗ DAC output after deletion is incorrect! This indicates the bug!\n");
    ...
    printf("Expected behavior: After deleting Node 1->Node 4, DAC should output 5.0\n");
