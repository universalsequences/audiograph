...
void test_hot_swap_basic() {
    ...
    // Now hot swap with a new NUMBER node that outputs 25.0
    float *new_state = malloc(NUMBER_MEMORY_SIZE * sizeof(float));
    new_state[NUMBER_VALUE] = 25.0f;
    
    // Create hot swap operation structure
...
void test_hot_swap_bounds_checking() {
    ...
    float *state = malloc(NUMBER_MEMORY_SIZE * sizeof(float));
    state[NUMBER_VALUE] = 100.0f;
...
void test_replace_keep_edges_basic() {
    ...
    bool connect_result = connect(lg, num1, 0, gain, 0);
    ...
    float *new_gain_state = malloc(GAIN_MEMORY_SIZE * sizeof(float));
    new_gain_state[GAIN_VALUE] = 5.0f; // New gain value
...
void test_replace_keep_edges_port_shrinking() {
    ...
    bool connect1 = connect(lg, num1, 0, gain, 0);
    bool connect2 = connect(lg, num2, 0, gain, 0);
    ...
    float *replacement_state = malloc(NUMBER_MEMORY_SIZE * sizeof(float));
    replacement_state[NUMBER_VALUE] = 100.0f;
...
void test_hot_swap_stress() {
    ...
    for (int i = 0; i < 4; i++) {
        bool connect_result = connect(lg, nodes[i], 0, nodes[i+1], 0);
        assert(connect_result);
    }
    ...
    for (int i = 1; i < 5; i++) {
        float *new_state = malloc(GAIN_MEMORY_SIZE * sizeof(float));
        new_state[GAIN_VALUE] = new_gains[i-1];
...
void test_retire_drain_system() {
    ...
    for (int i = 0; i < 4; i++) {
        // Create new state
        float *new_state = malloc(NUMBER_MEMORY_SIZE * sizeof(float));
        new_state[NUMBER_VALUE] = values[i];
        ...
        bool swap_result = apply_hot_swap(lg, &hot_swap);
        assert(swap_result);
        printf("✓ Applied hot swap %d (value: %.1f)\n", i+1, values[i]);
