 ...
 *
 * Tests orphan detection across key scenarios with proper queue-based API usage.
 * Validates the back-pointer-based O(V+E) orphan detection algorithm.
 ******************************************************************************/
...
static bool validate_specific_nodes(LiveGraph *lg, int *node_ids, bool *expected, int count, const char *test_name) {
  bool passed = true;
...
static void print_all_nodes(LiveGraph *lg, const char *scenario) {
  printf("\n--- %s: %d nodes, DAC=%d ---\n", scenario, lg->node_count, lg->dac_node_id);
...
static bool test_basic_connectivity(void) {
  ...
  // Connect osc1 path
  connect(lg, osc1, 0, gain, 0);
  connect(lg, gain, 0, dac, 0);
  // osc2 not connected
  ...
  bool result = validate_specific_nodes(lg, test_nodes, expected, 4, "Basic Connectivity");
...
static bool test_no_dac(void) {
  ...
  for (int i = 0; i < lg->node_count; i++) {
    ...
    if (i == 0) continue; // DAC was at node 0
...
static bool test_diamond_topology(void) {
  ...
  // Create diamond
  connect(lg, osc, 0, gain1, 0);
  connect(lg, osc, 0, gain2, 0);
  connect(lg, gain1, 0, mixer, 0);
  connect(lg, gain2, 0, mixer, 1);
  connect(lg, mixer, 0, dac, 0);
  ...
  bool result = validate_specific_nodes(lg, test_nodes, expected, 5, "Diamond Topology");
...
static bool test_fan_out(void) {
  ...
  bool expected[] = {false, false, false, false, true, false}; // Only gain3 orphaned
...
static bool test_performance(void) {
  ...
  for (int i = 0; i < 4; i++) {
    connect(lg, nodes[i], 0, nodes[i+1], 0);
  }
  connect(lg, nodes[4], 0, lg->dac_node_id, 0);
  ...
  for (int i = 0; i < 100; i++) {
    disconnect(lg, nodes[0], 0, nodes[1], 0);
    connect(lg, nodes[0], 0, nodes[1], 0);
    apply_graph_edits(lg->graphEditQueue, lg);
  }
